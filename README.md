...
* Note a utilização da palavra-chave de Python, ou _keyword_ em inglês, `while`. Essa construção repete o bloco de código que estiver 'dentro' dela enquando a condição lógica passada a ela seja avaliada como verdadeira, ou `True`. Como no caso já foi colocado o valor `True` fixo, o bloco de código será repetido para sempre. Para colocar uma ou mais linhas de código 'dentro' do `while`, colocam-se 4 espaços no início de cada uma das linhas. Esses 4 espaços são conhecidos como indentação. ou seja, as linhas são 'indentadas' e com isso passam a ser 'comandadas' pela ação do `while`.
* Utiliza-se aqui também a palavra-chave de Python `if`. Essa construção executa o bloco de código indentado a ela caso a condição lógica passada a ela seja avaliada como verdadeira, ou `True`. Caso contrário o bloco indentado não será executado.
